 ...
 * audit_hint2.js
 *
 * Audits all hint 2s in puzzles.json against the rules in HINT_ARCHITECTURE_RULES.md.
 *
 * Full audit:   node audit_hint2.js
 * Single test:  node audit_hint2.js --hint "..." --category fork --url "..." --group 0
 */
...
/**
 * Run all hint-2 checks.
 * @param {string} hint2         - The hint 2 text.
 * @param {string} hint1         - The hint 1 text (for "repeats-hint1" check).
 * @param {object} ctx
 ...
 */
function checkHint2(hint2, hint1, { category, game, move }) {
  const failures = [];
  const sentences = getSentences(hint2);
  const lower = String(hint2).toLowerCase();
  ...
  const sqId = String(hint2).match(/\b[a-h][1-8]\b/i);
  if (sqId) {
    failures.push({ code: 'square-id', detail: `contains "${sqId[0]}"` });
  }

  const punct = String(hint2).match(/[;:]/);
  if (punct) {
  ...
  if (s2) {
    ...
    if (bannedS2Names) {
      ...
      const hit = bannedS2Names.find(kw => s2lower.includes(kw.toLowerCase()));
      if (hit) {
        failures.push({
          code: 'opening-name-in-s2',
          detail: `sentence 2 must not name the opening — found "${hit}" (describe the board pattern instead)`,
        });
      }
    }
  }

  // ── category must be named in sentence 1 ──
  ...
  // ── hint 2 sentence 1 must not be identical to hint 1 sentence 1 ──

  if (hint1 && sentences.length >= 1) {
    const h1Sentences = getSentences(hint1);
    const h1s1 = (h1Sentences[0] || '').toLowerCase().trim();
    const h2s1 = (sentences[0] || '').toLowerCase().trim();
    if (h1s1 && h2s1 && h1s1 === h2s1) {
      failures.push({
        code: 'repeats-hint1',
        detail: 'sentence 1 is identical to hint 1\'s sentence 1 — must restate the principle differently',
      });
    }
  }
  ...
  Object.entries(data).forEach(([category, catData]) => {
    ...
    Object.entries(catData.puzzles).forEach(([url, puzzle]) => {
      ...
      puzzle.moveHints.forEach((group, groupIndex) => {
        ...
        entries.push({ category, url, groupIndex, hint1: hint1 || '', hint2, game, move });
      });
    });
  });
  return entries;
...
function runFullAudit(data) {
  ...
  entries.forEach(({ category, hint2 }) => {
    const sentences = getSentences(hint2);
    const s2 = sentences[1] || '';
    if (!s2) return;
  ...
  entries.forEach(({ category, url, groupIndex, hint1, hint2, game, move }) => {
    total++;
    const failures = checkHint2(hint2, hint1, { category, game, move });
    ...
    if (failures.length > 0) {
      allFailures.push({ category, url, groupIndex, hint1, hint2, failures });
      failures.forEach(f => { codeCount[f.code] = (codeCount[f.code] || 0) + 1; });
    }
  });

  console.log('\n=== HINT 2 AUDIT REPORT ===\n');
  console.log(`Total hint 2s checked:     ${total}`);
  ...
  const shown = {};
  allFailures.forEach(({ category, url, groupIndex, hint2, failures }) => {
    failures.forEach(f => {
...
function runSingleTest({ hint2, hint1, category, url, groupIndex, data }) {
  const fen = parseFenFromLichessUrl(url);
  let game = null;
  let move = null;
  if (fen) {
...
function main() {
  ...
  const hint2Idx    = args.indexOf('--hint');
  const hint1Idx    = args.indexOf('--hint1');
  ...
  if (hint2Idx !== -1) {
    runSingleTest({
      hint2:      args[hint2Idx + 1],
      hint1:      hint1Idx !== -1 ? args[hint1Idx + 1] : '',
